...
1. In app.rs
   a. get SubscriptionLink from Subscriptions
   b. define FeedType {Rss, Atom, }; based on a
   c. if Rss { mod rss } else if Atom { mod atom }
   d. each mod w/ respective crate parsers
   e. try to compose and DRY generalize
